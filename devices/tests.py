...
class DevicesViewsTestCase(TestCase):
    def setUp(self):
        ...
        self.device2 = devices.objects.create(
            device_brand="Test Brand 2", device_model="Test Model 2", os_version="2"
        )
    ...
    def test_create_device_with_invalid_input(self):
        ...
        response = self.client.post(
            reverse("devices:create_device"),
            {"device_brand": "", "device_model": "", "os_version": "1"},
        )
    ...
    def test_create_device_access_as_non_admin(self):
        ...
        )  # Should return redirect or forbidden status

    # Testing an admin can delete a device
    def test_delete_device_as_admin(self):
        # Log in as admin
        self.client.login(username="admin", password="12345")
        # Create URL for delete operation
        url = reverse("devices:delete_device", args=[self.device1.pk])
        # Saving number of devices before deletion for comparison later
        device_count_before = devices.objects.count()
        # Performing the request
        response = self.client.post(url)
        # Checking for redirection to the all_devices page after deletion
        self.assertRedirects(response, reverse("devices:all_devices"))
        # Asserting that the device count has decreased by one
        self.assertEqual(devices.objects.count(), device_count_before - 1)

    # Testing non staff user can not delete device
    def test_delete_device_access_as_non_admin(self):
        ...
        # Performing the request
        response = self.client.post(url)
        # Check that the non staff user is redirected or forbidden from deleting
        self.assertNotEqual(response.status_code, 200)
        # Assert the device still exists after the non staff user attempted to delete it
        self.assertTrue(devices.objects.filter(pk=self.device1.pk).exists())
